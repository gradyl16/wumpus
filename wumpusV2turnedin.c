...
void checkForKill(struct GameInfo *gameInfo,  // Hazards and game info
                  int currentArrowRoom,       // Room arrow is currently traveling through
...
{
  // Wumpus is shot
  if(currentArrowRoom == gameInfo->wumpusRoom)
  {
    *wumpusIsAlive = false;
  }
  // Person is shot
  else if(currentArrowRoom == gameInfo->personRoom)
  {
    *personIsAlive = false;
  }
}// end checkForKill(...)
...
int main(void)
{
    ...
    int currentArrowRoom;       // Used to check for kill during arrow traversal on an 'S' type move
    ...
    {   
      ...
      else if(typeOfMove == 'S')
      {
        ...
        {
          currentArrowRoom = gameInfo.personRoom;
          ...
          for( int i = 0; i < arrowDisplacement; i++ )
          {
            if(roomIsAdjacent(Rooms[ currentArrowRoom ], nextArrowRoom))
            {
              currentArrowRoom = nextArrowRoom;
              checkForKill(&gameInfo, currentArrowRoom, &personIsAlive, &wumpusIsAlive);
              if(!wumpusIsAlive)
              {
                // Arrow killed Wumpus
            ...
            {
              currentArrowRoom = Rooms[ currentArrowRoom ][0];
          ...
          gameInfo.arrowRoom = currentArrowRoom;
